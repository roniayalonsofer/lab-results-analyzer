...
if page == "soil":
    ...
    with st.expander("🔶 מילוי ידני של ערכי סף (LOQ, LOD, MRL, MDL) — אופציונלי", expanded=False):
        ...
        <div class="override-banner">
          ⚠️ <b>אופציונלי</b> — למלא רק אם רוצים.<br>
          הערכים ישמשו <b>רק</b> כשבקובץ המקורי חסר ערך סף מתאים.<br><br>
          לדוגמה: בקובץ TPH של אל-כם יש ערך LOQ אך אין ערך LOD.<br>
          ערך ה-LOD קיים רק בגרסת ה-PDF של אותו דוח.<br><br>
    ...
    if excel_ok:
        ...
        _thresh_files = [
            ("⬇️ ערכי סף TIER1 + VSL (קרקע)", "ערכי_סף_מעודכנים_גרסה_7_דצמבר_2024.xlsx", "thresh_dl_main"),
            ("⬇️ ערכי סף PFAS",              "ערכי_סף_PFAS_גרסה_3.xlsx",                "thresh_dl_pfas"),
            ("⬇️ ערכי סף מי תהום",           "ערכי_סף_מי_תהום.xlsx",                    "thresh_dl_gw"),
        ]
        _thresh_cols = st.columns(len(_thresh_files))
        for _col, (_label, _fname, _key) in zip(_thresh_cols, _thresh_files):
            _fpath = os.path.join(THRESH_DIR, _fname)
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        '<td><span class="badge badge-green">PDF</span><span class="badge badge-blue">Excel</span></td>'
        '<td>מי תהום, קרקע כימיה (חנקן, פוספאט, TOC), קרקע חיידקים (ספירה כללית)</td></tr>'
